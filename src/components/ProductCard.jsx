...
const ProductCard = ({ ele ,addToCartHandler}) => {
    ...
    return (
        <div onClick={() => navigate(`/pd/${ele._id}`)} className='h-96 cursor-pointer grid place-items-start gap-2 drop-shadow-4xl rounded-curve bg-white py-2 px-4 pb-4 relative'>
            ...
            <img src={`${ele?.photos[0].url}`} alt={`${ele?.name}-img`} className="m-auto h-52 w-52 mix-blend-multiply bg-[#F2F2F2] " />
            ...
            <section className='flex justify-between items-center w-full'>
                ...
                <button onClick={(e) => addToCartHandler(e, {...ele,photo:ele?.photos[0].url})} className='flex  items-center justify-center w-32 h-9 rounded-lg border-2 bg-gradient-to-r from-green-400 to-add-to-cart-seagreen p-1 text-xs font-bold'>Add to Cart <span> <IoIosArrowForward /></span> </button>
            </section>
        </div>
    )
}
